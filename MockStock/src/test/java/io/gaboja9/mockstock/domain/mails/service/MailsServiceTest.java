...
public class MailsServiceTest {
    ...
    void setUp() {
        ...
        testMail = new Mails("Test Subject","Test Content", false, LocalDateTime.now(), testMember);
        testMail2 = new Mails("Test Subject2","Test Content2", true, LocalDateTime.now(), testMember);

        testDto = MailsResponseDto.builder()
        ...
        testDto2 = MailsResponseDto.builder()
    ...
    void getAllMails_정상동작() {
        ...
        when(mailsRepository.findByMembersId(testMember.getId())).thenReturn(List.of(testMail, testMail2));
        when(mailsMapper.toDto(List.of(testMail, testMail2))).thenReturn(List.of(testDto, testDto2));
    ...
    void getMailsByReadStatus_정상동작() {
        ...
        verify(membersRepository, times(1)).findById(memberId);
        verify(mailsRepository, times(1)).findByMembersIdAndReadStatus(testMember.getId(), readStatus);
        verify(mailsMapper, times(1)).toDto(anyList());
    }
    ...
    void getMailsByReadStatus_유저없음예외() {
        ...
        assertThrows(NotFoundMemberException.class, () -> mailsService.getMailsByReadStatus(memberId, true));
