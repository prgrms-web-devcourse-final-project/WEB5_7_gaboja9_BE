...
class StocksServiceTest {
    ...
    void initData_success() {
        ...
        // 특정 주식들이 저장되는지 확인
        verify(stocksRepository).save(argThat(stock ->
                "삼성전자".equals(stock.getStockName()) && "005930".equals(stock.getStockCode())));
        verify(stocksRepository).save(argThat(stock ->
                "NAVER".equals(stock.getStockName()) && "035420".equals(stock.getStockCode())));
        verify(stocksRepository).save(argThat(stock ->
                "카카오".equals(stock.getStockName()) && "035720".equals(stock.getStockCode())));
    }
    ...
    void getAllStocks_success() {
        // given
        List<Stocks> expectedStocks = Arrays.asList(
    ...
    void initData_ShouldCreateCorrectStockData() {
        ...
        for (String[] stockData : expectedStocks) {
            verify(stocksRepository).save(argThat(stock ->
                    stockData[0].equals(stock.getStockName()) && stockData[1].equals(stock.getStockCode())));
        }
    }
}
