...
public class StocksWeeklyChartService {
  ...
  public List<DailyStockPrice> getLatestWeeklyPrices(String stockCode, int limit) {
    ...
    try {
      ...
      List<DailyStockPrice> weeklyData = stocksWeeklyRepository.findStoredWeeklyPrices(stockCode,
          limit);
  ...
  public List<DailyStockPrice> getMorePastData(String stockCode, Instant beforeTimestamp,
    ...
    try {
      ...
      List<DailyStockPrice> weeklyData = stocksWeeklyRepository.findStoredWeeklyPricesBefore(
          stockCode, beforeTimestamp, limit);

      // 2. 데이터가 없으면, 일봉에서 실시간으로 집계합니다.
      if (weeklyData.isEmpty()) {
        log.info("저장된 과거 주봉 데이터 없음. 서비스에서 실시간 집계를 요청합니다. stockCode: {}", stockCode);
        return stocksWeeklyRepository.aggregateFromDailyBefore(stockCode, beforeTimestamp, limit);
      }
      return weeklyData;
  ...
  public List<DailyStockPrice> getMoreRecentData(String stockCode, Instant afterTimestamp,
    ...
    try {
      ...
      List<DailyStockPrice> weeklyData = stocksWeeklyRepository.findStoredWeeklyPricesAfter(
          stockCode, afterTimestamp, limit);

      // 2. 데이터가 없으면, 일봉에서 실시간으로 집계합니다.
      if (weeklyData.isEmpty()) {
        log.info("저장된 최신 주봉 데이터 없음. 서비스에서 실시간 집계를 요청합니다. stockCode: {}", stockCode);
        return stocksWeeklyRepository.aggregateFromDailyAfter(stockCode, afterTimestamp, limit);
      }
      return weeklyData;
  ...
  private void validateStockCode(String stockCode) {
    ...
    stocksRepository.findByStockCode(stockCode)
