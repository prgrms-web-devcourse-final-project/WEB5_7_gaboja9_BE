...
public interface OrdersControllerSpec {
    ...
    @Operation(
            summary = "주식을 지정가로 매수합니다.",
            description = "주식을 지정가로 매수하고 매수 결과를 불러옵니다.",
            responses =
            @ApiResponse(
                    responseCode = "201",
                    description = "매수 결과를 보여줍니다.",
                    content =
                    @Content(
                            mediaType = "application/json",
                            schema =
    ...
    ResponseEntity<OrderResponseDto> executeLimitBuy(
            @Valid @RequestBody OrdersLimitTypeRequestDto requestDto,
            @AuthenticationPrincipal MembersDetails membersDetails
    );

    @Operation(
            summary = "주식을 지정가로 매도합니다.",
            description = "주식을 지정가로 매도하고 매도 결과를 불러옵니다.",
            responses =
            @ApiResponse(
                    responseCode = "201",
                    description = "매도 결과를 보여줍니다.",
                    content =
                    @Content(
                            mediaType = "application/json",
                            schema =
    ...
    ResponseEntity<OrderResponseDto> executeLimitSell(
            @Valid @RequestBody OrdersLimitTypeRequestDto requestDto,
            @AuthenticationPrincipal MembersDetails membersDetails
    );
