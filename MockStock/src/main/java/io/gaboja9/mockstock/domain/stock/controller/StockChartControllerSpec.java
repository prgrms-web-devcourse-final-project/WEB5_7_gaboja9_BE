...
public interface StockChartControllerSpec {
  ...
  Map<String, Object> loadPastDailyChartData(
      ...
      @PathVariable @NotBlank String stockCode,
      @Parameter(description = "기준 시점 (이 시점보다 과거 데이터 조회)", required = true, example = "2025-07-07T00:00:00Z")
      @RequestParam("before") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) Instant beforeTimestamp,
  ...
  Map<String, Object> loadRecentDailyChartData(
      @Parameter(description = "주식 코드", required = true, example = "005930")
      @PathVariable @NotBlank String stockCode,
      @Parameter(description = "기준 시점 (이 시점보다 미래 데이터 조회)", required = true, example = "2025-07-08T00:00:00Z")
      @RequestParam("after") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) Instant afterTimestamp,
  ...
  @Operation(
      summary = "분봉 차트 과거 데이터 조회",
      description = "분봉 차트에서 왼쪽 드래그시 과거 데이터를 추가로 로드합니다."
  )
  ...
  @Operation(
      summary = "분봉 차트 최신 데이터 조회",
      description = "분봉 차트에서 오른쪽 드래그시 최신 데이터를 추가로 로드합니다."
  )
  @ApiResponses(value = {
      @ApiResponse(
          responseCode = "200",
  ...
  Map<String, Object> getInitialWeeklyChartData(
      ...
      @RequestParam(defaultValue = "52") @Positive int limit
  );

  @Operation(
      summary = "주봉 차트 과거 데이터 조회",
      description = "차트 왼쪽 드래그시 과거 데이터를 추가로 로드합니다."
  )
  Map<String, Object> loadPastWeeklyChartData(
      @Parameter(description = "주식 코드", required = true, example = "005930")
      @PathVariable @NotBlank String stockCode,
      @Parameter(description = "기준 시점 (이 시점보다 과거 데이터 조회)", required = true, example = "2025-07-14T00:00:00Z")
      @RequestParam("before") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) Instant beforeTimestamp,
      @Parameter(description = "조회할 데이터 개수", example = "26")
      @RequestParam(defaultValue = "26") @Positive int limit
  );

  @Operation(
      summary = "주봉 차트 최신 데이터 조회",
      description = "차트 오른쪽 드래그시 최신 데이터를 추가로 로드합니다."
  )
  Map<String, Object> loadRecentWeeklyChartData(
      @Parameter(description = "주식 코드", required = true, example = "005930")
      @PathVariable @NotBlank String stockCode,
      @Parameter(description = "기준 시점 (이 시점보다 미래 데이터 조회)", required = true, example = "2025-07-21T00:00:00Z")
      @RequestParam("after") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) Instant afterTimestamp,
