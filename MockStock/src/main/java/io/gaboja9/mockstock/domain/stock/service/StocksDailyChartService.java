...
public class StocksDailyChartService {

  private final StocksDailyRepository stocksDailyRepository;
   ...
   */
  public List<DailyStockPrice> getLatestDailyPrices(String stockCode, int limit) {
    validateStockCode(stockCode);
    validateLimit(limit);

    try {
      return stocksDailyRepository.findLatestDailyPrices(stockCode, limit);
    } catch (Exception e) {
      log.error("Failed to fetch latest daily prices for stock: {}", stockCode, e);
  ...
  public List<DailyStockPrice> getMorePastData(String stockCode, Instant beforeTimestamp,
    ...
    try {
      return stocksDailyRepository.findDailyPricesBefore(stockCode, beforeTimestamp, limit);
    } catch (Exception e) {
      log.error("Failed to fetch past daily prices for stock: {} before {}", stockCode,
          beforeTimestamp, e);
      throw StockChartException.dataFetchFailed(stockCode, e);
    }
  }
  ...
  public List<DailyStockPrice> getMoreRecentData(String stockCode, Instant afterTimestamp,
    ...
    try {
      return stocksDailyRepository.findDailyPricesAfter(stockCode, afterTimestamp, limit);
    } catch (Exception e) {
      log.error("Failed to fetch recent daily prices for stock: {} after {}", stockCode,
          afterTimestamp, e);
      throw StockChartException.dataFetchFailed(stockCode, e);
    }
  }
