...
public class StocksBulkService {
  ...
  public void fetchAllStocksAllData(String marketCode) {
    ...
    log.info(" 전체 주식 데이터 수집 시작 - 총 {}개 종목 ({}개씩 병렬 처리)",
        allStocks.size(), MAX_CONCURRENT_STOCKS);
  ...
  private void processSingleStock(StockResponse stock, String marketCode, String longTermStartStr,
      ...
      AtomicInteger processedCount, int totalSize) {
    ...
    try {
      ...
      log.error("[{}/{}] {} 종목 처리 중 오류 발생: {}", currentCount, totalSize, stockCode, e.getMessage());
    }
  }
